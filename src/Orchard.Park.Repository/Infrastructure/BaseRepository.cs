...
using Orchard.Park.Model.Messages.Basics;
using SqlSugar;
...
namespace Orchard.Park.Repository.Infrastructure
{
    public abstract class BaseRepository<T> : IBaseRepository<T> where T : class, new()
    {
        ...
        protected BaseRepository(IUnitOfWork unitOfWork)
        {
            ...
            DbBaseClient = unitOfWork.GetDbClient();
        }

        private SqlSugarScope DbBaseClient { get; }

        protected SqlSugarScope DbClient => DbBaseClient;
        ...
        /// <param name="selectExpresion"></param>
        /// <param name="orderBy"></param>
        /// <param name="blUseNoLock"></param>
        /// <returns></returns>
        public async Task<List<TResult>> QueryListByClauseAsync<TResult>(Expression<Func<T, bool>> predicate,
            Expression<Func<T, TResult>> selectExpresion, bool iswhere = false, Expression<Func<T, bool>> where = null, string orderBy = "", bool blUseNoLock = false)
        {
            return await DbBaseClient
                .Queryable<T>()
                .OrderByIF(!string.IsNullOrEmpty(orderBy), orderBy)
                .WhereIF(predicate != null, predicate)
                .WhereIF(where != null && iswhere, where)
                .WithNoLockOrNot(blUseNoLock)
        ...
        /// <returns></returns>
        public async Task<bool> UpdateAsync(List<T> entity, Expression<Func<T, object>> columns)
        {
            return await DbBaseClient.Updateable(entity).UpdateColumns(columns).ExecuteCommandHasChangeAsync();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<int> UpdateOrAddAsync(List<T> entity)
        {
            return await DbBaseClient.Storageable(entity).ExecuteCommandAsync();
        }
        ...
        /// <returns></returns>
        public async Task<bool> UpdateAsync(T entity, Expression<Func<T, bool>> where, Expression<Func<T, object>> ignoreColumn)
        {
            return await DbBaseClient.Updateable(entity).IgnoreColumns(ignoreColumn).Where(where).ExecuteCommandHasChangeAsync();
        }
            ...
            bool blUseNoLock = false) where T1 : class, new()
        {
            return await DbBaseClient
                .Queryable(joinExpression)
                .WhereIF(where && whereLambda is not null, whereLambda)
                .Select(selectExpression)
                .WithNoLockOrNot(blUseNoLock)
                .ToListAsync();
        }
        ...
        /// <param name="blUseNoLock"></param>
        /// <returns></returns>
        public async Task<TResult> QueryMuchFirstAsync<TResult>(
        ...
        public async Task<List<TResult>> QueryMuchAsync<T1, T2, T3, TResult>(
            Expression<Func<T1, T2, T3, object[]>> joinExpression,
            Expression<Func<T1, T2, T3, TResult>> selectExpression,
            Expression<Func<T1, T2, T3, bool>> whereLambda1 = null,
            bool where = false,
            Expression<Func<T1, T2, T3, bool>> whereLambda = null,
            bool blUseNoLock = false) where T1 : class, new()
        {
            return await DbBaseClient
                .Queryable(joinExpression)
                .WhereIF(whereLambda1 is not null, whereLambda1)
                .WhereIF(where && whereLambda is not null, whereLambda)
                .Select(selectExpression)
                .WithNoLockOrNot(blUseNoLock)
                .ToListAsync();
        ...
        {
            return await DbBaseClient.Queryable<T>().AnyAsync(where);
        }
    }
}